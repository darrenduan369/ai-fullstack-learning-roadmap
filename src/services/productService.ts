...
export function getStockStatus(product: Readonly<Product>): StockStatus {
  if (product.stock === 0) {
    return "out-of-stock";
...
export function toProductCards(products: readonly Product[]): ProductCard[] {
...
export function createProductMap(
  ...
  return products.reduce<Partial<Record<number, Product>>>((map, product) => {
...
export function filterProducts(
  products: readonly Product[],
  { category, minPrice, featuredOnly = false }: ProductFilterOptions = {},
): Product[] {
...
export function calculateSelectedStock(...products: Product[]): number {
  return products.reduce((total, product) => total + product.stock, 0);
}

export function getPriceRange(products: readonly Product[]): [number, number] {
